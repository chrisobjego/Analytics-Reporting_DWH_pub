...
renewal_counts AS (
    SELECT
        ...
        CAST(COUNT(DISTINCT CASE WHEN invoice_status NOT IN ('voided', 'not_paid') THEN invoice_id END) - 1 AS INT64) AS number_of_renewals_excl_voided_not_paid,
        CAST(COUNT(DISTINCT CASE WHEN invoice_status NOT IN ('voided', 'not_paid') THEN invoice_id END) AS INT64) AS number_of_subscriptions_cycles,
        CASE
            WHEN (COUNT(DISTINCT CASE WHEN invoice_status NOT IN ('voided', 'not_paid') THEN invoice_id END) - 1) > 0 THEN 'yes'
            ELSE 'no'
        END AS renewal_status,
        SUM(CASE WHEN invoice_status NOT IN ('voided', 'not_paid') THEN amount_excl_vat ELSE 0 END) AS total_revenue_excl_voided_not_paid
    FROM
        source_data
    GROUP BY
        user_id
