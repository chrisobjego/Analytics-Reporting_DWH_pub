...
renewal_counts AS (
    SELECT
        ...
        CAST(COUNT(DISTINCT CASE WHEN invoice_status NOT IN ('voided', 'not_paid','payment_due') THEN invoice_id END) - 1 AS INT64) AS number_of_renewals_excl_voided_not_paid,
        ...
        SUM(CASE WHEN invoice_status NOT IN ('voided', 'not_paid','payment_due') THEN amount_excl_vat ELSE 0 END) AS total_revenue_excl_voided_not_paid
    FROM
        source_data
    GROUP BY
        user_id
...
SELECT  
    ...
    pc.count_Premium_12,
    tr.total_revenue,
    pc_excl_voided.count_Basic_3 AS count_Basic_3_excl_voided,
    pc_excl_voided.count_Premium_3 AS count_Premium_3_excl_voided,
    pc_excl_voided.count_Basic_12 AS count_Basic_12_excl_voided,
    pc_excl_voided.count_Premium_12 AS count_Premium_12_excl_voided,
